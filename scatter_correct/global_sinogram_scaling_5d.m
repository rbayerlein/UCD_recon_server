...
TOFbyTOF_scaling=true; % determines whether to use global
...
disp('Finding global scaling factor (takes about 3 minutes)...');
tic
...
%         x_val = sum(sum(sum(sum(data_pd_slice)))) / sum(sum(sum(sum(data_s_t_slice))));
    ...
    func = @(x) GetImsse(data_pd, data_s_5d + data_t_5d, x); %#ok<UNRCH>
