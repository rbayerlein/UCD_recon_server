...
handles.node_num_str = fname_choose(ind_ext+1)
...
if strcmp(handles.os_type, 'unix') > 0.5
  ...
  for acq_num = 1:2:69
    ...
    outfolder = [handles.path_choose, get(handles.outfolder_name, 'String'), '_',num2str(acq_num),'/'];
    [succ, x, y] = mkdir(outfolder);
    if succ < 0.5
      errordlg('Could not create output folder'); 
      return
    ...
    infile_fname = [handles.fname_choose_base, num2str(fnum), '.raw']; 
    infile_fname = [infile_fname(1:(end-11)), num2str(acq_num), infile_fname((end-9):end)]; 
    infile_fullpath = fullfile(handles.path_choose, infile_fname); 
    %infile_fullpath = ['"',infile_fullpath,'"']; 
   
    
    fname_config = ['../read_lm/Reconstruction_Parameters_',num2str(acq_num)];
    fid_config = fopen(fname_config, 'w'); 
    ...
    if thread_count < (handles.num_threads - 0.5)
      sh_str_temp = ['../read_lm/process_lm_sino_explorer_singlefile  ',fname_config, ' & ']; 
    else
      sh_str_temp = ['../read_lm/process_lm_sino_explorer_singlefile  ',fname_config, ' & wait; '];
      thread_count = 0; 
    end
...
% nframes = round(nframes); 
 ...
 %for fn = 0:(num_frames-1)
  %
 % 	sh_str = [sh_str, '../read_lm/sum_sino ']; 
 % 	for kk = 1:8
 % 		ff = [outfolder, 'sinogram_block_f',num2str(fn),'_prompts.',num2str(kk),'.raw ']; 
 % 		sh_str = [sh_str, ff]; 
 % 	end
 % 
 % 	sh_str = [sh_str, ' & ']; 
 ...
 % 	sh_str = [sh_str, '../read_lm/sum_sino ']; 
 % 	for kk = 1:8
 % 		ff = [outfolder, 'sinogram_block_f',num2str(fn),'_randoms.',num2str(kk),'.raw ']; 
 % 		sh_str = [sh_str, ff]; 
 % 	end
  %
 % 	sh_str = [sh_str, ' & wait; ']; %
...
%	sh_str = []; 
%
%	sh_str = ['rm -r ',outfolder, 'sinogram_block_f',num2str(fn),'*.*.raw'];
%	system(sh_str); 
%	sh_str = []; 
%	%sh_str = [sh_str, ff]; 
%
...
%
 % 	sh_str = [sh_str, '../read_lm/sum_sino ']; 
 % 	for kk = 1:8
 % 		ff = [outfolder, 'sinogram_ssrb_f',num2str(fn),'_prompts.',num2str(kk),'.raw ']; 
 % 		sh_str = [sh_str, ff]; 
 % 	end
 % 
 % 	sh_str = [sh_str, ' & ']; 
 ...
 % 	sh_str = [sh_str, '../read_lm/sum_sino ']; 
 % 	for kk = 1:8
 % 		ff = [outfolder, 'sinogram_ssrb_f',num2str(fn),'_randoms.',num2str(kk),'.raw ']; 
 % 		sh_str = [sh_str, ff]; 
 % 	end
 % 
 % 	sh_str = [sh_str, ' & wait; ']; 
...
%	sh_str = []; 
%
%	sh_str = ['rm -r ',outfolder, 'sinogram_ssrb_f',num2str(fn),'*.*.raw'];
%	system(sh_str); 	
%	sh_str = []; 
 % end
