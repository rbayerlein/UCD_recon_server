	...
	//for (long long nn = 0; nn < num_events_p; nn++) {
		...
		if (r_num < p1_prob && notdone1) {
			for (int k = 0; k < 5; k++) {
				...
				outfile.write(reinterpret_cast<const char *>(&din), sizeof(short)); 
			}

			infile_s1.read(reinterpret_cast<char *>(&din_f), sizeof(float)); 
			outfile_s.write(reinterpret_cast<const char *>(&din_f), sizeof(float)); 

			infile_m1.read(reinterpret_cast<char *>(&din_f), sizeof(float)); 
			outfile_m.write(reinterpret_cast<const char *>(&din_f), sizeof(float)); 
