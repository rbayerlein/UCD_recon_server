...
int main(int argc, char** argv) {
	...
	string sens_image_fullpath; 
	sens_image_fullpath = argv[2]; 
	...
	// Image must be 256 x 256 x 830 (2.344 isotropic)
        ...
        string roi_image_fullpath = fdir_code;
        roi_image_fullpath.append("roi_mask_256x256x830.img"); 
	...
	// read sensitivity image
	// sensitiivity image must be 256 x 256 x 830
	// 

	ifstream sens_img; 
	sens_img.open(sens_image_fullpath.c_str(), ios::in | ios::binary); 

	if (!sens_img) {
		cout << sens_image_fullpath << "\n"; 
		cout << "invalid sensitivity image!\n"; 
		return 1; 
	}
	sens_img.seekg(0, sens_img.end); 
	int sens_img_size = sens_img.tellg() / sizeof(float);

	if (sens_img_size != 256*256*830) {
		cout << "invalid sensitivity image size\n"; 
		return 1; 
	}

	sens_img.seekg(0, sens_img.beg); 

	vector<float> sens_img_lut(256*256*830); 
	for (int q = 0; q < (256*256*830); q++) {
		sens_img.read(reinterpret_cast<char *>(&sens_img_lut[q]), sizeof(float)); 
		if (sens_img_lut[q] < 0.000001) {
			sens_img_lut[q] = 1.0; 
		}
	}
	...
	int histo_img_size = 256*256*830; 
	vector<float> histo_image(histo_img_size); 
	double c_xx, c_xx_temp, c_yy, c_yy_temp, c_zz, c_zz_temp, v_xx, v_yy, v_zz, l_v = 0.0; 
	...
	int num_cod_tof_bins = 1; 
	vector<double> cod_tof_bins_wts(1);
        vector<double> cod_tof_bins(1);
	cod_tof_bins_wts[0] = 1.0;
	cod_tof_bins[0] = 0.0; 
	float sens_norm = 0.0; 

	/*
	int num_cod_tof_bins = 7; 
	vector<double> cod_tof_bins_wts(7); 
	vector<double> cod_tof_bins(7); 
	cod_tof_bins_wts[0] = 0.0043;
	cod_tof_bins_wts[1] = 0.0538;
	cod_tof_bins_wts[2] = 0.2422;
	cod_tof_bins_wts[3] = 0.3994;
	cod_tof_bins_wts[4] = 0.2422;
	cod_tof_bins_wts[5] = 0.0538;
	cod_tof_bins_wts[6] = 0.0043;

	cod_tof_bins[0] = -600.0;
	cod_tof_bins[1] = -400.0;
	cod_tof_bins[2] = -200.0;
	cod_tof_bins[3] = 0.0;
	cod_tof_bins[4] = 200.0;
	cod_tof_bins[5] = 400.0;
	cod_tof_bins[6] = 600.0;
				...
				if (ind>0) {
					...
					if (prompt) {
						...
						if (write_cod) {
							...
							for (int tof_i = 0; tof_i < num_cod_tof_bins; tof_i++) {
								tof_x = (0.3 * (t_v + cod_tof_bins[tof_i]) / 2.0) * v_xx;
								tof_y = (0.3 * (t_v + cod_tof_bins[tof_i]) / 2.0) * v_yy;
								tof_z = (0.3 * (t_v + cod_tof_bins[tof_i]) / 2.0) * v_zz;
								c_xx_temp = c_xx + tof_x;
								c_yy_temp = c_yy + tof_y;
								c_zz_temp = c_zz + tof_z; 
								// check if event is inside mask image
								pix_xd = 128.0 + (c_xx_temp / 2.344); 
								pix_yd = 128.0 + (-1.0 * c_yy_temp / 2.344);
								pix_zd = (c_zz_temp / 2.344);
								if (pix_xd < 0) {
									pix_xd  = 0.0; 
								}
								if (pix_yd < 0) {
								...
								sens_norm = sens_img_lut[img_index];
								if (sens_norm < 0.1) {
									sens_norm = 0.1; 
								}
								if (sens_norm > 10.0) {
								...
								if (roi_val > 0) {
									x_mean[unit_z] = x_mean[unit_z] + (c_xx_temp);
									y_mean[unit_z] = y_mean[unit_z] + (c_yy_temp);
									z_mean[unit_z] = z_mean[unit_z] + (c_zz_temp); 

									xyz_mean_counts[unit_z] = xyz_mean_counts[unit_z] + (1.0); // * cod_tof_bins_wts[tof_i]); // / sens_img_lut[img_index]);
									histo_image[img_index] = histo_image[img_index] + 1.0; 
								} 
