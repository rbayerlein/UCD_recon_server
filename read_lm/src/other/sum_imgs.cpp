...
int main(int argc, char** argv) {
	...
	vector<float> sino_sum(sino_size); 
	...
	for (long k = 0; k < sino_size; k++) {
		sino_in1.read(reinterpret_cast<char *>(&din), sizeof(float));
		sino_sum[k] = sino_sum[k] + din; 
		din = 0; 

		if (argc > 2) {
			sino_in2.read(reinterpret_cast<char *>(&din), sizeof(float));
			sino_sum[k] = sino_sum[k] + din; 
			din = 0; 
		}
		if (argc > 3) {
			sino_in3.read(reinterpret_cast<char *>(&din), sizeof(float));
			sino_sum[k] = sino_sum[k] + din; 
			din = 0; 
		}
		if (argc > 4) {
			sino_in4.read(reinterpret_cast<char *>(&din), sizeof(float));
			sino_sum[k] = sino_sum[k] + din; 
			din = 0; 
		}
		if (argc > 5) {
			sino_in5.read(reinterpret_cast<char *>(&din), sizeof(float));
			sino_sum[k] = sino_sum[k] + din; 
			din = 0; 
		}
		if (argc > 6) {
			sino_in6.read(reinterpret_cast<char *>(&din), sizeof(float));
			sino_sum[k] = sino_sum[k] + din; 
			din = 0; 
		}
		if (argc > 7) {
			sino_in7.read(reinterpret_cast<char *>(&din), sizeof(float));
			sino_sum[k] = sino_sum[k] + din; 
			din = 0; 
		}
		if (argc > 8) {
			sino_in8.read(reinterpret_cast<char *>(&din), sizeof(float));
			sino_sum[k] = sino_sum[k] + din; 
			din = 0; 
		}
	...
	for (long j = 0; j < sino_size; j++) {
		sino_out.write(reinterpret_cast<const char *>(&sino_sum[j]), sizeof(float));
	}
	sino_out.close(); 
