			...
			if (coinc_tag) {
				...
				if (ind>0) {
					...
					if (prompt) {
						...
						phiA = 2.0 * PI * (double)transA / (double)num_crystals;
						phiB = 2.0 * PI * (double)transB / (double)num_crystals;

						phiphi = phiB - phiA; 
						if (phiphi > PI) {
							phiphi = (2.0 * PI) - phiphi; 
						}
						fan_angle = (PI - phiphi) / 2.0; 
						//cout << fan_angle << "\n"; 
						
						if (unit_diff == 0 && unitA == 3 && fan_angle < max_fan_angle) {
							tof_store[crys1] = tof_store[crys1] + t_v; 
							tof_store[crys2] = tof_store[crys2] - t_v; 
							counts_store[crys1] = counts_store[crys1] + 1.0; 
							counts_store[crys2] = counts_store[crys2] + 1.0;
						} 
