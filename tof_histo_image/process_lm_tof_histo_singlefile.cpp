...
int main(int argc, char** argv) {
	...
	int histo_img_size = 256 * 256 * 648; 
	...
	while (run) {
		...
		if (!(din1<=a32) && !(din2>=a32)) {
			...
			if (bk1 == 0 && bk2 == 1 && bk3 == 0) {
				...
				block_rate[ind] = (double)br_temp * 10.0;				
				
				//cout << "blkX: " << blkX << ", blkY: " << blkY << ", singles: " << block_rate[ind] << " / sec\n"; 
				//unitid[0] = ui0; 
				//unitid[1] = ui1; 
				//unitid[2] = ui2;
				//unitid[3] = ui3; 
				
				//cout << "ui0 = " << ui0 << ", ui1 = " << ui1 << ", ui2 = " << ui2 << ", ui3 = " << ui3 << ", unit id = " << uid << "\n"; 
			...
			if (bk1 == 0 && bk2 == 0 && bk3 == 0) {
				...
				//if (1 ) {
						//cout << infile.tellg() << "\n"; 
					//cout << "year = " << year1 << ", month = " << month1 << ", day = " << day1 << ", hour = " << hour1 << ", minute = " << minute1 << ", second = " << second1 << "\n"; 
				//}
				...
				//if (!time_tag) {
					...
					//cout << "Frame start: " << year1 << "/" << month1 << "/" << day1 << " " << hour1 << ":" << minute1 << ":" << second1 << "." << milli0 << "\n";
					
					// update frame end datetime
				...
				if (ind>0) {
					...
					if (prompt) {
						...
						if (write_histo_img) {
							...
							p_xx = round(c_xx / 2.344); 
							p_yy = round(c_yy / 2.344); 
							p_zz = round(c_zz / 3.0); 
							
							if ( (abs(p_xx) < 128) && (abs(p_yy) < 128) && (abs(p_zz) < 324) ) {
								p_xx = p_xx + 128; 
								p_yy = p_yy + 128;
								p_zz = p_zz + 324; 
								ind_histo = p_yy + (256*p_xx) + (256*256*p_zz); 								
...
// ssrb sino names
			...
			/* 
			// do random mean estimation
			if (rand_sub) {
				outfile_fullpath_p = "";
				outfile_fullpath_s = ""; 
				outfile_fullpath_s_nontof = ""; 
				stringstream ss;
				ss << "lm_reorder_f" << frame_num;
				fname_out = ss.str();
				outfile_fullpath_p = outfolder;
				outfile_fullpath_s = outfolder;
				outfile_fullpath_s_nontof = outfolder;
				outfile_fullpath_p.append(fname_out);
				outfile_fullpath_s.append(fname_out);
				outfile_fullpath_s_nontof.append(fname_out); 
				
				outfile_fullpath_p.append("_prompts.lm");
				outfile_fullpath_s.append("_sub.lm"); 
				outfile_fullpath_s_nontof.append("_sub_nontof.lm"); 
				...
				ofstream outfile_s_nontof; 
				outfile_s_nontof.open(outfile_fullpath_s_nontof.c_str(), ios::out | ios::binary); 

				outfile_fullpath_p = "";
				outfile_fullpath_s = ""; 
				outfile_fullpath_s_nontof = ""; 
				
				if (!infile_p || !outfile_s) {
					infile_p.close();
					cout << "Cannot open prompts listmode file for randoms correction\n";
				}


				infile_p.seekg(0, infile_p.end);
				...
				for (long jj = 0; jj < num_events; jj++) {
					...
					ind = ct1 + (840 * rt1) + (sec2 * 570360);
					lorsum1 = sector_flood[ind];

					ind = ct2 + (840 * rt2) + (sec1 * 570360);
					lorsum2 = sector_flood[ind];

					rtemp = ((lorsum1 * lorsum2) / lorsumall);
					
					outfile_s_nontof.write(reinterpret_cast<const char *>(&rtemp), sizeof(float)); 
					
					unit_diff = (int)abs(unitA - unitB); 
					t_window = coinc_window[unit_diff]; 
			...
			*/
			...
			// start reconstruction for frame xx if run_recon flag is on
			if (run_recon) {
				stringstream ss_runrecon; 
				ss_runrecon << "sh " << outfolder << "/run_lmrecon_explorer_f" << frame_num << ".sh"; 
				runrecon_str = ss_runrecon.str(); 
				cout << "\n" << runrecon_str << "\n"; 
				ss_runrecon << "";
				ss_runrecon.clear();
				
				const char *command = runrecon_str.c_str(); 
				...
				/*
				stringstream ss_removeraws; 
				ss_removeraws << "sh " << outfolder << "/remove_raws_f" << frame_num << ".sh"; 
				removeraws_str = ss_removeraws.str(); 
				cout << "\n" << removeraws_str << "\n"; 
				ss_removeraws << "";
				ss_removeraws.clear();
				
				const char *command2 = removeraws_str.c_str(); 
					
				cout << "\nRemoving extra raw files...\n"; 
				system(command2); 
				removeraws_str = ""; 
				*/
